...
def exp_direct(dataset, seed=DEFAULT_SEED):
    ...
    model_file_name = f'./models/direct_1_{seed}.wts.h5'
    print('Training on the source data...')
    history = model.fit(
                  train_generator, 
                  validation_data=val_generator,
...
def exp_gradual(dataset, diff_dataset, n_samples=3000, seed=DEFAULT_SEED):
    ...
    model_file_name = f'./models/gradual_s_{n_samples}_{seed}.h5'
    print('Training on the source data...')
    history = model.fit(
                  train_generator, 
                  validation_data=val_generator,
    ...
    model_file_name = f'./models/gradual_i1_{n_samples}_{seed}.h5'
    ...
    model_file_name = f'./models/gradual_i2_{n_samples}_{seed}.h5'
...
def main(**kwargs):
    ...
    with open(f'./adapt_results_{seeds[0]}.pkl', 'wb') as fh:
    ...
    with open(f'./adapt_results_{seeds[0]}.csv', 'w') as file:
